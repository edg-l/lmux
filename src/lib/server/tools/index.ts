	...
	const tools: ToolDefinition[] = [
		...
		},
		{
			...
			function: {
				name: 'render_html',
				description:
					"Render an interactive HTML page seamlessly in the chat. The output is displayed directly inline, not in a box or embed. IMPORTANT STYLING RULES: Do NOT set any background-color on body, html, or wrapper containers. Do NOT create card/container divs with backgrounds or shadows. The page inherits the app's dark theme automatically. Just place your content directly in <body> without wrappers.\n\nPre-loaded CSS toolkit: native dark theme with cyan accent. All form elements (input, range, select, button) are pre-styled to match the app. Utility classes: .flex, .flex-col, .grid, .items-center, .justify-between, .gap-1 to .gap-4, .p-1 to .p-4, .text-center, .w-full, .text-sm/.text-lg/.text-xl, .font-bold, .text-muted, .text-accent, .rounded, .border, .bg-surface, .btn-accent. CSS vars: --color-accent, --color-surface, --color-border, --color-text-primary, --color-text-secondary.\n\nChart.js: <canvas id='chart'> + new Chart(document.getElementById('chart'), { type: 'bar'|'line'|'pie', data: {labels:[...], datasets:[{data:[...]}]} })\n\nThree.js: const scene = new THREE.Scene(); const renderer = new THREE.WebGLRenderer();\n\nAll scripts/styles must be inline. No relative URLs.",
				parameters: {
					type: 'object',
					properties: {
						html: {
